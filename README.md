...
Built on the original Streamlit prototype (`app.py`) — the model logic is unchanged, exposed via FastAPI, and consumed by a Vite + React + TypeScript frontend.
...
```
.
├── core.py              ← model + parsing logic (extracted from app.py)
...
## Keeping the Streamlit prototype

`app.py` is left untouched and still runs with `streamlit run app.py` if you want to compare side-by-side. It imports nothing from `core.py` — the extraction left it self-contained.
